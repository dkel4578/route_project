...
import { fetchDatePage } from "../redux/slices/dateSlice.js";
...
import DateModal from "../modals/DateModal"; // ✅ 모달 import

ModuleRegistry.registerModules([AllCommunityModule]);

const DatePage = () => {
  const dispatch = useDispatch();
  const { loading, error, data } = useSelector((state) => state.date);

  const [modalOpen, setModalOpen] = useState(false);
  const [editRow, setEditRow] = useState(null);

  useEffect(() => {
    dispatch(fetchDatePage());
  }, [dispatch]);
  ...
  const columnDefs = useMemo(
    () => [
      { headerName: "날짜코드", field: "날짜코드", flex: 1 },
      { headerName: "날짜", field: "날짜", flex: 1 },
      { headerName: "년도", field: "년도", flex: 1 },
      { headerName: "분기", field: "분기", flex: 1 },
      { headerName: "월", field: "월", flex: 1 },
      { headerName: "월(영문)", field: "월영문", flex: 1 },
      {
        headerName: "액션",
        flex: 1,
        cellRenderer: (params) => (
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="max-w-6xl mx-auto mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 mb-2">📊 날짜 내역</h1>
          <p className="text-gray-600">
            날짜 데이터를 한눈에 확인하고 관리할 수 있습니다.
          </p>
        </div>
        <Button
          type="primary"
